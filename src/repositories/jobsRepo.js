...
function create(jobNumber, data, containers = []) {
  const tx = db.transaction(() => {
    const cols = ['job_number', ...EDITABLE];
    const values = [jobNumber, ...EDITABLE.map((c) => (data[c] === undefined ? null : data[c]))];
...
function update(id, data) {
  ...
  const set = EDITABLE.map((c) => `${c} = ?`).join(', ');
  const values = EDITABLE.map((c) => (data[c] === undefined ? null : data[c]));
